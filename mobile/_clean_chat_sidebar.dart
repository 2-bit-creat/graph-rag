...
class _ChatSidebarState extends State<ChatSidebar> {
  ...
  Future<void> _rename(String id, String? current) async {
    ...
    final title = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('채팅�??�름 변�?),
        content: TextField(
          controller: ctrl,
          autofocus: true,
          decoration: const InputDecoration(
              hintText: '?�름', border: OutlineInputBorder(), isDense: true),
  ...
  Future<void> _delete(String id) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('채팅�???��'),
        content: const Text('??채팅방을 ??��?�까?? 지?�그?�프???��??�요.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx, false), child: const Text('취소')),
          FilledButton(
              onPressed: () => Navigator.pop(ctx, true), child: const Text('??��')),
        ],
      ),
    );
  ...
  String? _resolvedPreview(String? preview, {required bool active}) {
    ...
    if (!active || !p.contains('?�기 처리')) return p;
          ...
          child: Row(
            children: [
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => const [
                  PopupMenuItem(value: 'rename', child: Text('?�름 변�?)),
