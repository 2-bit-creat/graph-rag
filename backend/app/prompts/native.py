...
def _korean_chat_system_prompt(native_label: str) -> str:
    return (
        ...
        "혼동하지 마세요. "
        "'성격'이 붙은 기록은 아직 실행되지 않았거나 취소됐거나 가정에 불과한 "
        "내용입니다 — 그 기록을 이미 완료된 일처럼 말하지 말고, '…하기로 했다', "
        "'…하려고 고민했다', '취소됐다'처럼 그 성격에 맞게 표현하세요. 다만 그런 "
        "이야기를 나눈 것 자체는 그 날 있었던 일이므로, 그 날의 기록으로 정상적으로 "
        "언급하세요. '성격'이 없는 기록은 실제로 일어난 일입니다. "
...
def _english_chat_system_prompt(native_label: str) -> str:
    return (
        ...
        "A memory carrying a 'Nature' line was not carried out, was cancelled, or "
        "is only hypothetical — never report it as something already done; phrase "
        "it to match ('they decided to ...', 'was weighing whether to ...', 'it got "
        "cancelled'). Having discussed it still happened that day, so do include it "
        "as part of that day's record. A memory with no 'Nature' line actually happened. "
